mod audio_meta;
use audio_meta::{scan_and_extract, AudioMetadata, ScanResult, update_metadata_logic, update_metadata_batch_logic, convert_image_to_png_logic};
...
async fn scan_directory(path: String) -> Result<ScanResult, String> {
    let result = scan_and_extract(&path);
    Ok(result)
}
...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![greet, scan_directory, update_metadata, update_metadata_batch, process_and_embed_artwork, get_cover_art, convert_image_to_png])
