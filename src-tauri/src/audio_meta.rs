...
fn process_file(
    ...
    if let Some(t) = tag {
        ...
        // Xử lý ảnh bìa
        let picture = t.pictures().iter().find(|p| p.pic_type() == PictureType::CoverFront || p.pic_type() == PictureType::Other);
        if let Some(pic) = picture {
            meta.has_cover = true;
            if let Some(album_str) = &meta.album {
                if !albums_map.contains_key(album_str) || albums_map.get(album_str).unwrap().is_none() {
                    let base_name = album_str.replace(|c: char| {
                        c == '<' || c == '>' || c == ':' || c == '"' || c == '/' || c == '\\' || c == '|' || c == '?' || c == '*'
                    }, "_");
        ...
        if let Some(album_str) = &meta.album {
            if !albums_map.contains_key(album_str) || (extracted_cover_path.is_some() && albums_map.get(album_str).unwrap().is_none()) {
...
pub fn scan_and_extract(dir_path: &str) -> ScanResult {
    ...
    #[cfg(windows)]
    {
        ...
        let _ = Command::new("attrib").arg("+h").arg(&dora_meta_dir).status();
    }
        ...
        if path.is_file() {
            if let Some(ext) = path.extension().and_then(|s| s.to_str()) {
                ...
                if allowed_extensions.contains(&ext_lower.as_str()) {
                    match process_file(path, dir_path, &mut albums_map, &mut artists_set, &mut genres_set, &re_feat) {
    ...
    if let Ok(entries) = fs::read_dir(&covers_dir) {
        for entry in entries.filter_map(|e| e.ok()) {
            let path = entry.path();
            if path.is_file() {
                if let Some(file_name) = path.file_name().map(|os_str| os_str.to_string_lossy().to_string()) {
    ...
    for (album_name, cover_path) in albums_map {
        albums.push(AlbumInfo { album_name, cover_path });
    }
    albums.sort_by_key(|a| a.album_name.to_lowercase());
    
    let aggregated = AggregatedMetadata { artists, albums, genres };
...
pub fn update_metadata_logic(files: Vec<String>, metadata: &AudioMetadata) -> Result<(), String> {
    let errors: Vec<String> = files.par_iter().filter_map(|file| {
        ...
        for t_type in tag_types {
            if let Some(tag) = tagged_file.tag_mut(t_type) {
                ...
                if let Some(genre) = &metadata.genre {
                    if genre.trim().is_empty() { tag.remove_genre(); } else { tag.set_genre(genre.clone()); }
                }
                if let Some(year) = metadata.year {
                    if year == 0 { tag.remove_year(); } else { tag.set_year(year); }
                }
            }
        }
...
pub fn update_metadata_batch_logic(updates: Vec<AudioMetadata>) -> Result<(), String> {
    let errors: Vec<String> = updates.par_iter().filter_map(|update| {
...
pub fn process_and_embed_artwork_logic(files: Vec<String>, image_path: &str) -> Result<(), String> {
    let picture = if !image_path.is_empty() {
        ...
        resized.write_to(&mut png_data, image::ImageFormat::Png).map_err(|e| e.to_string())?;
...
pub fn convert_image_to_png_logic(image_path: &str) -> Result<String, String> {
    let img = if image_path.starts_with("data:image/") {
        ...
        let image_bytes = base64::engine::general_purpose::STANDARD.decode(parts[1]).map_err(|e| format!("Lỗi giải mã Base64: {}", e))?;
        image::load_from_memory(&image_bytes).map_err(|e| format!("Không thể đọc ảnh từ Base64: {}", e))?
    } else {
    ...
    img.save_with_format(&temp_path, image::ImageFormat::Png).map_err(|e| format!("Lỗi lưu ảnh PNG: {}", e))?;
